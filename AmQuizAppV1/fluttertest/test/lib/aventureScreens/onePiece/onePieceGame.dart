...
class OnePieceGame extends StatefulWidget {
  ...
  @override
  State<OnePieceGame> createState() => _OnePieceGameState();
}

class _OnePieceGameState extends State<OnePieceGame> {
  // Timer
  ...
  late Future<DocumentSnapshot>
      userDataFuture; // Déclarer un Future pour les donnée
  ...
  void initializeQuestions() {
    ...
    questionsFuture =
        DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid)
            .getQuestionsAventure("onepiece", widget.fichierJson);
  }
    ...
    if (indexQuestion == maxQuestion) {
      ...
      if (realScore > widget.record) {
        await DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid)
            .updateRecordsOnePiece(widget.numeroIle, realScore);
      }
      await DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid)
          .debloqueProchaineIleOnePiece(widget.numeroIle, realScore);

      showResult(context, maxQuestion + 1, score, widget.fichierJson,
          widget.nomIle, widget.record, widget.numeroIle, "onepiece");
    } else {
      indexQuestion += 1;
      progress.value = 0;
      tempsActuel = 0;
            ...
            children: [
              Row(
                children: [
                  ...
                  InkWell(
                    onTap: () {
                      askLeaveAventure(context); // Revenir à la page précédente
                    },
                    child: Container(
